...
#'
#' @return a plot showing the mean abundance vs. coefficient of variation (standard deviation divided by mean value) of all occurred cells. Cells with high mean abundance and/or high variance will be labeled.
#'
#' @export
show.cellsofinterest = function(nums,occurred.cells){
  occurrednums = nums[occurred.cells,]
  aver = apply(occurrednums,1,mean)
  cv = apply(occurrednums,1,function(x){sd(x)/mean(x)})
  #logratio = log(cv/aver)

  avertops = order(aver,decreasing = T)[1:10]
  cvtops = order(cv,decreasing = T)[1:10]
  #ratiotops = order(abs(logratio),decreasing = T)[1:10]
  #tolabel = union(union(avertops,cvtops),ratiotops)
  tolabel=union(avertops,cvtops)

  p = ggplot()+
    geom_point(aes(x=aver,y=cv))+
    geom_point(aes(x=aver[tolabel],y=cv[tolabel]),color="red")+
    geom_text_repel(aes(x=aver[tolabel],y=cv[tolabel],label=names(aver)[tolabel]), size=3,
                    arrow = arrow(length = unit(0.01, "npc"), type = "open", ends = "last"))+
    labs(x="mean abundance",y="coefficient of variation")+
    theme_classic()
