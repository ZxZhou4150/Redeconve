...
coloc.network=function(corr,thresh,cell.type,annotations,ntypes,color=NULL){
  diag(corr)=0
  corr[corr<thresh]=0
  g = graph_from_adjacency_matrix(corr,mode="undirected",weighted=T)
...
#' Extracting subgraph
#' 
#' This function extracts a subgraph, including a chosen node and its all neighbors, from the co-localization network generated by `coloc.network`.
#' 
#' @param g The "igraph" object, result of `coloc.network`.
#' @param node The node to be chosen. The name of that cell or cell type.
...
extract_subgraph = function(g, vertex_of_interest, ...){
  ...
  # setting the layout
  layout <- matrix(ncol=2, nrow=vcount(subg)-1)
  layout[,1] <- cos(seq(0, 2*pi, length.out = vcount(subg)))[-1]
  layout[,2] <- sin(seq(0, 2*pi, length.out = vcount(subg)))[-1]
  
  # adding an offset to avoid overlap
  offset_range <- 0.1  # define the range for the random offset
  set.seed(123)  # Setting a seed for reproducibility
  for (i in 1:(vcount(subg)-1)) {
    layout[i,] <- layout[i,] + runif(2, -offset_range, offset_range)
  }
  
  center_vertex_id <- which(V(subg)$name == vertex_of_interest)
  if(center_vertex_id == 1){
    layout = rbind(c(0,0),layout)
  }
  else if(center_vertex_id == vcount(subg)){
    layout = rbind(layout,c(0,0))
  }
  else
    layout = rbind(layout[1:(center_vertex_id-1),],c(0,0),layout[center_vertex_id:(vcount(subg)-1),])
  
  E(subg)$color <- "grey"
  E(subg)[from(vertex_of_interest)]$color = "red"
  
  # Plot the subgraph
  plot(subg, 
       layout=layout,
       edge.width = E(subg)$weight * 5, # Adjust multiplier as needed for visibility
       edge.label = round(E(subg)$weight, 2), # Show edge weights
       # edge.label.cex = 0.8, # Adjust label size as needed
       vertex.label = V(subg)$name,
       main = sprintf("%s and its neighbors", vertex_of_interest),
       ...)
  return(subg)
}
